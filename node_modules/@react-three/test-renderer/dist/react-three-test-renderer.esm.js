...
const create = async (element, options) => {
  ...
  const _store = _roots.get(canvas).store;
  ...
  return {
    ...
    getInstance() {
      ...
      if (!_roots.has(canvas)) return null;

      // Traverse fiber nodes for R3F root
      const root = {
        current: _roots.get(canvas).fiber.current
      };
    ...
    async update(newElement) {
      if (!_roots.has(canvas)) return console.warn('RTTR: attempted to update an unmounted root!');
...
export { index$1 as ReactThreeTest, act, create, index as default };
