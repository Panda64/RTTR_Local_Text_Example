...
const toTreeBranch = obj => obj.map(child => {
  return treeObjectFactory(lowerCaseFirstLetter(child.type || child.constructor.name), {
    ...child.__r3f.memoizedProps
  }, toTreeBranch([...(child.children || []), ...child.__r3f.objects]));
});
const toTree = root => root.children.map(obj => treeObjectFactory(lowerCaseFirstLetter(obj.type), {
...
class WebGL2RenderingContext {
  constructor(canvas) {
    ...
    for (const method of functions) {
      var _this$method;
      (_this$method = this[method]) != null ? _this$method : this[method] = () => {};
    }
...
const createCanvas = ({
  ...
  var _globalThis$WebGLRend, _globalThis$WebGL2Ren;
  let canvas;
  ...
  beforeReturn == null ? void 0 : beforeReturn(canvas);
  class WebGLRenderingContext extends WebGL2RenderingContext {}
  // @ts-ignore
  (_globalThis$WebGLRend = globalThis.WebGLRenderingContext) != null ? _globalThis$WebGLRend : globalThis.WebGLRenderingContext = WebGLRenderingContext;
  // @ts-ignore
  (_globalThis$WebGL2Ren = globalThis.WebGL2RenderingContext) != null ? _globalThis$WebGL2Ren : globalThis.WebGL2RenderingContext = WebGL2RenderingContext;
  return canvas;
};
...
const create = async (element, options) => {
  ...
  const _root = fiber.createRoot(canvas).configure({
    frameloop: 'never',
    ...options,
    events: undefined
  });
  const _store = fiber._roots.get(canvas).store;
  await act(async () => _root.render(element));
  const scene = _store.getState().scene;
  return {
    ...
    getInstance() {
      ...
      if (!fiber._roots.has(canvas)) return null;

      // Traverse fiber nodes for R3F root
      const root = {
        current: fiber._roots.get(canvas).fiber.current
      };
      while (!((_root$current$child = root.current.child) != null && _root$current$child.stateNode)) {
        var _root$current$child;
        root.current = root.current.child;
      }
    ...
    async update(newElement) {
      if (!fiber._roots.has(canvas)) return console.warn('RTTR: attempted to update an unmounted root!');
    ...
    async advanceFrames(frames, delta = 1) {
